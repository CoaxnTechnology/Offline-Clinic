...
class Config:
    ...
    SQLALCHEMY_DATABASE_URI = os.getenv("DATABASE_URL") or "sqlite:///clinic.db"
    ...
    DICOM_MAX_FILE_SIZE = int(
        os.getenv("DICOM_MAX_FILE_SIZE", "104857600")
    )  # 100MB default
    DICOM_CONNECTION_TIMEOUT = int(
        os.getenv("DICOM_CONNECTION_TIMEOUT", "30")
    )  # seconds
    DICOM_MAX_CONCURRENT_STORES = int(os.getenv("DICOM_MAX_CONCURRENT_STORES", "10"))
    DICOM_ENABLE_COMPRESSION = (
        os.getenv("DICOM_ENABLE_COMPRESSION", "true").lower() == "true"
    )
    DICOM_STORAGE_QUOTA_GB = int(
        os.getenv("DICOM_STORAGE_QUOTA_GB", "100")
    )  # 100GB default
...
class ProductionConfig(Config):
    ...
    if not SECRET_KEY or SECRET_KEY == "dev-secret-key-change-in-production":
        raise ValueError(
            "SECRET_KEY environment variable must be set in production and must not be the default value"
        )
