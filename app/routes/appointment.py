...
def list_appointments():
    ...
    if doctor:
        query = query.filter(
            Appointment.doctor.ilike(f"%{doctor}%")
...
def get_appointment(appointment_id):
    ...
    has_dicom = DicomImage.query.filter_by(appointment_id=appointment.id).count() > 0
    return jsonify(
        {
            "success": True,
            "data": {
...
def create_appointment():
    ...
    patient = (
        Patient.query.filter_by(id=data["patient_id"])
        .filter(Patient.deleted_at.is_(None))
        .first()
    )
    if not patient:
    ...
    if current_user and current_user.role == "doctor":
        ...
        if clinic_id:
            clinic_doctor = Admin.query.filter_by(
                clinic_id=clinic_id, role="doctor", is_active=True
            ).first()
        if clinic_doctor:
    ...
    if not doctor_name:
        return jsonify(
            {"success": False, "error": "No active doctor found for this clinic"}
        ), 400
    ...
    if len(time_str) != 5 or time_str[2] != ":":
        return jsonify(
            {"success": False, "error": "Invalid time format. Use HH:MM (e.g., 10:30)"}
        ), 400
    ...
    try:
        ...
        db.session.rollback()
        return jsonify(
            {"success": False, "error": f"Failed to create appointment: {str(e)}"}
        ), 500
...
def update_appointment(appointment_id):
    ...
    try:
        ...
        db.session.rollback()
        return jsonify(
            {"success": False, "error": f"Failed to update appointment: {str(e)}"}
        ), 500
...
def delete_appointment(appointment_id):
    ...
    try:
        ...
        db.session.rollback()
        return jsonify(
            {"success": False, "error": f"Failed to delete appointment: {str(e)}"}
        ), 500
...
def schedule_appointments():
    ...
    if not isinstance(data, list):
        return jsonify(
            {"success": False, "error": "Request body must be an array of appointments"}
        ), 400

    if len(data) == 0:
        return jsonify(
            {"success": False, "error": "Appointments array cannot be empty"}
        ), 400
        ...
        try:
            ...
            for field in required_fields:
                if not apt_data.get(field):
                    errors.append(
                        {"index": idx, "error": f'Field "{field}" is required'}
                    )
                    continue
            ...
            try:
                ...
                errors.append(
                    {"index": idx, "error": "Invalid date format. Use YYYY-MM-DD"}
                )
                continue
            ...
            if existing:
                errors.append({"index": idx, "error": "Appointment already exists"})
                continue
    ...
    return jsonify(
        {
            ...
            "data": {"created": created, "errors": errors if errors else []},
        }
    ), 201
